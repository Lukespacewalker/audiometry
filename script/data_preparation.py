...
def initialize_data():
    def extract_patient_name(df: pd.DataFrame):
        t = pd.concat([df["patient_name"].str.extract(
            r"(?P<title>นาย|นาง สาว|นางสาว|นาง|.+\.|\S+\s+)(?P<patient_name>.+)", expand=True).apply(lambda col: col.str.strip()),
                       df.drop(columns=["patient_name"], inplace=False)], axis=1)
        return t
